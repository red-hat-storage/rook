...
func createOrUpdateNetworkPolicy(ctx gocontext.Context, context *clusterd.Context, np *networkingv1.NetworkPolicy, ownerInfo *k8sutil.OwnerInfo) error {
	...
	if err != nil {
		...
		// The policy already exists; fetch it to get the current ResourceVersion for the update.
		existing, getErr := context.Clientset.NetworkingV1().NetworkPolicies(np.Namespace).Get(ctx, np.Name, metav1.GetOptions{})
		if getErr != nil {
