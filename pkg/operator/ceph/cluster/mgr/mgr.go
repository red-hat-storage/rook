...
func updateMgrCephxStatus(c *clusterd.Context, clusterInfo *cephclient.ClusterInfo, didRotate bool) error {
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if err == nil && len(mgrKeyType) == 1 {
			logger.Debugf("determined authoritative cephx key type for mgrs in cluster namespace %q is %q", clusterInfo.Namespace, mgrKeyType[0])
			cluster.Status.Cephx.Mgr.KeyType = cephv1.CephxKeyType(mgrKeyType[0])
		} else {
			logger.Infof("failed to determine authoritative cephx key type for mgrs in cluster namespace %q having key types [%v]: %v", clusterInfo.Namespace, mgrKeyType, err)
		}
